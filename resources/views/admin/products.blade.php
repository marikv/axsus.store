...
@section('content')
    ...
    <div class="modal fade itemModal " tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel123" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-scrollable" role="document">
            <div class="modal-content  bg-light">
                ...
                <div class="modal-body">
                    <form method="post" id="itemForm" enctype="multipart/form-data" action="/adm/addOrSaveProduct">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="mini_description" id="mini_description" rows="2"></textarea>
                        </div>
                        <div class="form-group">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" data-dismiss="modal" id="btnSubmit" class="btn btn-primary" onclick="ajaxSubmitForm('itemForm')" >Сохранить</button>
                </div>
            </div>
        </div>
    </div>

    <pre><? //print_r($tableData['data']); ?></pre>
    ...
    <table id="loadTable" class="table table-bordered bg-light table-hover">
        ...
        @foreach ($tableData['data'] as $row)
            <tr id="trProduct{{ $row['id'] }}">
                ...
                <td>
                    <? if (!empty($row) && $row['old_price']) {echo '<div class="old-price">'.$row['old_price'].' руб.</div>';} ?>
                    {{ ($row['price']?: 0) }} руб.
                </td>
                <td>{{ $row['order_by'] }}</td>
                <td>
                    ...
                    <button class="btn btn-danger" onclick="deleteProduct({{ $row['id'] }})"><i class="far fa-trash-alt"></i></button>
                </td>
            </tr>
